...
| Registration | 29 July–2 October 2026, according to the event page | Not registered |
...
No external application or submission has been completed on the entrant's behalf.

The immediate administrative action is to sign in to HackQuest and select **Start Register** on the official event page. Creating an account and accepting the organizer's terms must be done from the entrant's account. The in-person Founder House has a separate application; it is not required to enter the online Buildathon, and Buildathon winners receive a guaranteed Founder House place according to the official Luma listing.
...
|---|---|---|---|
| P0 | Registration is incomplete | An excellent product cannot be judged if it is not registered | Register on HackQuest now; complete the project submission later |
| P0 | The current live Site is private | Judges need a link that opens without the owner's session | Publish a public judge-accessible build before submission |
| P0 | Human-readable deal metadata is stored only in localStorage | A judge opening deal `0` on another device loses the title, milestone descriptions, and evidence context | Add a portable, verifiable evidence/terms pack that travels with the deal link or can be imported |
| P1 | The product opens directly into an operational dashboard | A first-time judge must infer the problem and value | Add a concise judge landing section: problem, three-step flow, why Arbitrum, and one-click live proof |
...
| P1 | The core concept is familiar | Innovation is an explicit criterion | Turn completed milestones into a portable, verifiable work record and make selective evidence disclosure a central feature |
...
| P2 | Final submission material is missing | Judges need reproducible proof | Create a public README, architecture diagram, test instructions, contract and transaction links, screenshots, and later a short demo |
...
1. **Portable proof pack:** make terms, milestone names, and selectively shared evidence readable on another device while verifying them against their onchain commitments.
2. **Judge mode:** add a simple landing explanation and a one-click path to the finished deal and its transaction proof.
3. **Contract v2 quality:** add liveness deadlines, broader tests, documented threat model, and public source verification.
4. **Portable work record:** show completed milestones as a worker history that another client can verify without trusting ScopePay.
5. **Public release and repository:** publish the judge build and source after secrets and disposable test accounts are removed.
6. **Validation:** test the complete flow with fresh wallets and collect five short user interviews.
7. **Submission package and demo:** write the final HackQuest entry and record the video only after the product and judge path are stable.
